...
## How to use the <AppIcon className='bg-zinc-950 rounded-lg w-4 h-4 inline' /> Koala

<AppIcon className='bg-zinc-950 rounded-lg w-4 h-4 inline' /> Koala is like your
friendly financial translator! Here's what it can understand:
...
number (e.g., `$20.50`, `€10`) and <AppIcon className='bg-zinc-950 rounded-lg w-4 h-4 inline' /> Koala will automatically understand it. You can even include a space!
...
- **Numbers with a currency code:** Numbers with a 3 letter currency code like
  ...
  <AppIcon className='bg-zinc-950 rounded-lg w-4 h-4 inline' /> Koala even fixes
  typos! So `USD100`, `$ 100`, `10usd`, `10 uSd`, `usD100` are all fine.
...
- **Write them verbally** (e.g., `10 thousand`). In this case `11.65 million` will
  ...
  - Supported symbols for <AppIcon className='bg-zinc-950 rounded-lg w-4 h-4 inline' /> Koala:
    - `k`: Thousand (`1,000`)
    - `m`: Million (`1,000,000`)
...
<AppIcon className='bg-zinc-950 rounded-lg w-4 h-4 inline' /> Koala can handle all
the math operations you need for your finances:
...
Now that we got through all that, try the full power of <AppIcon className='bg-zinc-950 rounded-lg w-4 h-4 inline' /> Koala below!
